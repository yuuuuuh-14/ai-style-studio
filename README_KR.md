...
- 콘텐츠/스타일 이미지 업로드 → Inception-v3 기반 최적화로 스타일 변환
...
| **통신 아키텍처** | **GraphQL (gqlgen)** · **SSE (Server-Sent Events)** |
| **ML** | TensorFlow 2.15+ (CPU · SavedModel) |
| **영상 처리** | Go Native (image/*) |
| **패키지 관리** | Yarn Berry (PnP) |
| **배포** | Docker Compose (Backend) · Cloudflare Pages (Frontend) |
...
│
├── frontend/
│   ├── app/                     # Remix 앱 소스 (routes, store, components)
│   ├── public/                  # 정적 자산
│   ├── package.json             # 의존성 설정
│   └── vite.config.js           # Vite 설정 (Remix)
│
...
# 서버 실행
cd backend
export CGO_LDFLAGS="-L/opt/homebrew/lib -ltensorflow"
export CGO_CFLAGS="-I/opt/homebrew/include"
go run server.go
```

### 🛰️ 핵심 통신 기술
이 프로젝트는 데이터의 정확성과 실시간 스트리밍을 모두 잡기 위해 **GraphQL**과 **SSE**를 사용합니다.
- **GraphQL**: 정밀한 데이터 쿼리 및 변환 요청 관리 (gqlgen)
- **SSE**: 실시간 변환 진행률 및 웹캠 프레임 스트리밍 (Go concurrency 활용)
